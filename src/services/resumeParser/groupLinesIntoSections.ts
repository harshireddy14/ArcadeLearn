 ...
 * Reference: https://github.com/xitanggg/open-resume/blob/main/src/app/lib/parse-resume-from-pdf/group-lines-into-sections.ts
 */
...
/**
 * Detect if a line is a section title using heuristics.
 */
const isSectionTitle = (line: Line, lineNumber: number) => {
  ...
  // Main heuristic: Check if text is double emphasized (bold AND all uppercase)
  // This is generally true for a well formatted resume
  if (isBold(textItem) && hasLetterAndIsAllUpperCase(textItem)) {
    return true;
  }

  // Fallback heuristic: Keyword matching
  // (This heuristic is not well tested and may not work well)
