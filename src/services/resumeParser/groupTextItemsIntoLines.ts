 ...
 * Reference: https://github.com/xitanggg/open-resume/blob/main/src/app/lib/parse-resume-from-pdf/group-text-items-into-lines.ts
 */

import { BULLET_POINTS } from "./lib/bulletPoints";
import type { TextItems, Line, Lines } from "@/types/resume";

/**
 * Group text items into lines. Returns an array where each position
 * contains text items in the same line of the PDF file.
 * 
 * This step tackles 2 main issues:
 * 1. Remove unwanted noises (merged broken text items)
 * 2. Build contexts and associations (group items into lines)
 */
export const groupTextItemsIntoLines = (textItems: TextItems): Lines => {
  const lines: Lines = [];

  // Group text items into lines based on hasEOL (End of Line)
  let line: Line = [];
  ...
  // Many PDF docs are not well formatted (e.g. converted from other formats).
  // This creates noises where a single text item is divided into multiple ones.
  // This step merges adjacent text items if their distance is smaller than
  // a typical char width to filter out those noises.
  ...
  for (let line of lines) {
    ...
    for (let i = line.length - 1; i > 0; i--) {
      ...
      if (distance <= typicalCharWidth) {
        // Add space if needed before merging
 ...
 * This accounts for some of those cases.
 */
...
/**
 * Return the width of a typical character.
 * 
